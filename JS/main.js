...
document.addEventListener("DOMContentLoaded", function () {
    let pokemonRandom = getRandom();
    getPokemon(pokemonRandom)
});

const buscador = document.getElementById("buscadorPokemon")
buscador.addEventListener('input', function () {
    let idPokemon = buscador.value
    console.log()
    if (idPokemon == '') {
        refrescar()
    }
    if (idPokemon > 0 || idPokemon < 851) {
        getPokemon(idPokemon)
    }
    if (idPokemon < 1 || idPokemon > 850 || typeof (idPokemon) == 'string') {
        mostrarError()
    }
})

const getRandom = () => {
    return Math.floor(Math.random() * 849) + 1
}
...
const getDataPokemon = async (idPokemon) => {
    try {
        const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${idPokemon}`)
        const data = await response.json()
        return data
...
const mostrarPokemon = async (dataPokemon) => {
    console.log(dataPokemon)

    let tipo = getTipo(dataPokemon);
    let tipoPokemon = htmlTipo(tipo);
    ...
    <div id="contenido" class="w-50 d-flex justify-content-center align-items-center" value="${dataPokemon.id}">
            ...
            <div id="foto" class="w-50 d-flex justify-content-center align-items-center">
                <div>
    ...
    let dataPokemonIzq = await getDataPokemon(getPokemonIzq(dataPokemon.id));

    let dataPokemonDer = await getDataPokemon(getPokemonDer(dataPokemon.id));

    let footer = document.getElementById("footer");
    footer.innerHTML = `
    <div class="btn-bottom">
        <button class="btn-pknum" id="btn-izquierdo" onclick="irPokemonIzq(${dataPokemonIzq.id})">
        ...
        <button class="btn-pknum" id="btn-derecho" onclick="irPokemonDer(${dataPokemonDer.id})">
            <div class="numero">N°</div>
...
const irPokemonIzq = (idPokemom) => {
    getPokemon(idPokemom)
}

const irPokemonDer = (idPokemom) => {
    getPokemon(idPokemom)
}
...
const getTipo = (pokemon) => {
    let tipo = [];
    for (let index = 0; index < pokemon.types.length; index++) {
        tipo[index] = pokemon.types[index].type.name
    }
    return tipo
}

const htmlTipo = (tipoPokemon) => {
    let htmlTypePokemon = "";
    for (let index = 0; index < tipoPokemon.length; index++) {
        switch (tipoPokemon[index]) {
            case 'grass':
...
const mostrarError = () => {
    let datosPokemon = document.getElementById("contenidoPokemon");
    datosPokemon.innerHTML = `
    <div id="contenido" class="w-50 d-flex justify-content-center align-items-center">
        <div>
            <div>
                <h1>
                Error 404
    ...
    footer.innerHTML = `
    <div class="btn-bottom">
        <button class="btn-pknum" id="btn-izquierdo">
            <span class="material-symbols-outlined"></span>
            <div class="numero">N°</div>
            <div id="noPokemonIzq" class="numero"></div>
        </button>
        <img class="uni" src="./img/pokemonunite.png">
        <button class="btn-pknum" id="btn-derecho">
            <div class="numero">N°</div>
            <div id="noPokemonDer" class="numero"></div>
            <span class="material-symbols-outlined"></span>
        </button>
    </div>
    `;
